...
{
        /* initialize the mutex waiting Q and let the mutex holder to be NULL */
...
kmutex_lock(kmutex_t *mtx)
{
    ...
    if(mtx->km_holder != NULL){
        // KASSERT(mtx->km_holder != NULL,("Mutex is not available"));
        sched_sleep_on(&mtx->km_waitq);
    }
    else {
        mtx->km_holder = curthr;
    } */
    KASSERT(curthr && curthr != mtx->km_holder && "Current thread is the holder!!!");
    if (mtx->km_holder) {
		sched_sleep_on(&mtx->km_waitq);
	} else { 
		mtx->km_holder = curthr;
